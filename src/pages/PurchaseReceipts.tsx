...
import { PackageSearch, Calendar, CheckCircle, Clock, Ban, Search } from 'lucide-react';
...
export function PurchaseReceipts() {
  ...
  const [colSearch, setColSearch] = useState({ name: '', date: '' });

  const fetchReceipts = async () => {
    setLoading(true);
    try {
      const response = await erpService.getStockEntries(1000);
      setReceipts(response.filter((e: any) => e.stock_entry_type === 'Material Receipt'));
    } catch { setReceipts([]); }
  ...
  const statusCounts = {
    all: receipts.length,
    draft: receipts.filter(e => e.docstatus === 0).length,
    submitted: receipts.filter(e => e.docstatus === 1).length,
  };
  ...
  const hasFilters = filter !== 'all' || colSearch.name || colSearch.date;

  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-slide-up">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Nhập mua hàng</h1>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 animate-slide-up stagger-1">
        ...
        {hasFilters && (
          <button onClick={() => { setFilter('all'); setColSearch({ name: '', date: '' }); setPage(0); }} className="text-xs text-red-500 hover:text-red-600 font-medium ml-1">
            Xóa lọc
          </button>
        )}
      </div>
      ...
      <div className="animate-slide-up stagger-2">
        {loading ? (
          ...
          <div className="card p-8 text-center">
            ...
            <p className="text-sm font-medium text-gray-500">Không tìm thấy phiếu nhập mua nào.</p>
          </div>
        ) : (
          <>
            <div className="card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="data-table">
                  ...
                  <tbody>
                    {paginated.map((rec, i) => (
                      <tr key={rec.name} className="animate-slide-up" style={{ animationDelay: `${i * 10}ms` }}>
                        <td className="font-mono font-semibold text-blue-600">{rec.name}</td>
                        <td className="text-sm text-gray-500">{rec.posting_date}</td>
                        <td>
                          {rec.docstatus === 1 ? (
