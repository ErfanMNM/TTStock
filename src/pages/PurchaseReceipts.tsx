...
import { PackageSearch, Calendar, CheckCircle, Clock, Ban } from 'lucide-react';
      ...
      <div className="animate-slide-up stagger-2">
        {loading ? (
          ...
          <div className="card p-8 text-center">
            ...
            <p className="text-sm font-medium text-gray-500">Chưa có phiếu nhập mua nào.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {filtered.map((rec, i) => (
              <div key={rec.name} className="card p-4 animate-slide-up" style={{ animationDelay: `${i * 20}ms` }}>
                <div className="flex items-start space-x-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                    rec.docstatus === 1 ? 'bg-green-50' : rec.docstatus === 0 ? 'bg-yellow-50' : 'bg-red-50'
                  }`}>
                    <PackageSearch className={`w-5 h-5 ${
                      rec.docstatus === 1 ? 'text-green-500' : rec.docstatus === 0 ? 'text-yellow-500' : 'text-red-500'
                    }`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900">{rec.name}</p>
                    <div className="flex items-center space-x-2 mt-0.5">
                      <span className="chip chip-green text-[10px]">Nhập mua</span>
                    </div>
