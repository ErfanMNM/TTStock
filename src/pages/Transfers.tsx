...
import { ArrowLeftRight, Calendar, CheckCircle, Clock, Plus, Ban } from 'lucide-react';
...
export function Transfers() {
  ...
  const getTypeColor = (type: string) => {
    const colors: Record<string, string> = {
      ...
      'Material Receipt': 'chip-green',
      'Material Issue': 'chip-yellow',
    };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-slide-up">
        ...
        <Link to="/transfers/new" className="btn-primary !rounded-xl !px-4 !py-2.5 !text-sm w-full sm:w-auto justify-center">
      ...
      <div className="animate-slide-up stagger-2">
        {loading ? (
          ...
          <div className="card p-8 text-center">
            ...
            <p className="text-sm font-medium text-gray-500">Chưa có phiếu nhập xuất nào.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {filtered.map((entry, i) => (
              <div key={entry.name} className="card p-4 animate-slide-up" style={{ animationDelay: `${i * 20}ms` }}>
                <div className="flex items-start space-x-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                    entry.docstatus === 1 ? 'bg-green-50' : entry.docstatus === 0 ? 'bg-yellow-50' : 'bg-red-50'
                  }`}>
                    <ArrowLeftRight className={`w-5 h-5 ${
                      entry.docstatus === 1 ? 'text-green-500' : entry.docstatus === 0 ? 'text-yellow-500' : 'text-red-500'
                    }`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900">{entry.name}</p>
                    <div className="flex items-center space-x-2 mt-0.5">
                      <span className={`chip ${getTypeColor(entry.stock_entry_type)} text-[10px]`}>{getTypeLabel(entry.stock_entry_type)}</span>
                    </div>
